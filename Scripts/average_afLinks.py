...
txt_file_path = 'C:\\Users\\phili\\OneDrive - Universitaet Duisburg-Essen\\Bachelorarbeit\\Algo\\JSON\\avg2.txt'
...
# Hinzufügen eines globalen Bereichs für die Gesamtauswertung
results = {se: {
    'total_score': 0,
...
# Für die Gesamtwerte ohne Unterscheidung der Suchmaschine
overall_results = {
    'total_score': 0,
    'total_affiliate_links': 0,
    'total_highlight_strength': 0,
...
for entry in data:
    if isinstance(entry, list) and len(entry) > 0:
        ...
        if searchengine in results:
            ...
            # Nur Einträge berücksichtigen, die mindestens einen Affiliate-Link haben
            if affiliate_links > 0:
                ...
                # Berechnung für die Gesamtauswertung (ohne Suchmaschinen-Unterscheidung)
                overall_results['total_score'] += score
                overall_results['total_affiliate_links'] += affiliate_links
                overall_results['total_highlight_strength'] += highlight_strength
                overall_results['total_position'] += position
                overall_results['total_relScore'] += relScore
                overall_results['total_entries'] += 1

                if pos not in overall_results['pos_scores']:
                    overall_results['pos_scores'][pos] = {'score_sum': 0, 'count': 0}
                    overall_results['pos_affiliate_links'][pos] = {'affiliate_links_sum': 0, 'count': 0}

                overall_results['pos_scores'][pos]['score_sum'] += score
                overall_results['pos_scores'][pos]['count'] += 1
                overall_results['pos_affiliate_links'][pos]['affiliate_links_sum'] += affiliate_links
                overall_results['pos_affiliate_links'][pos]['count'] += 1

# Ergebnisse in einer Textdatei speichern
with open(txt_file_path, 'w', encoding='utf-8') as file:
    # Zuerst die spezifischen Werte für Google und Bing
    for se in search_engines:
    ...
    # Jetzt die Gesamtauswertung (ohne Unterscheidung der Suchmaschine)
    total_entries = overall_results['total_entries']
    # Durchschnittswerte berechnen
    average_score = overall_results['total_score'] / total_entries if total_entries > 0 else 0
    average_affiliate_links = overall_results['total_affiliate_links'] / total_entries if total_entries > 0 else 0
    average_highlight_strength = overall_results['total_highlight_strength'] / total_entries if total_entries > 0 else 0
    average_position = overall_results['total_position'] / total_entries if total_entries > 0 else 0
    average_relScore = overall_results['total_relScore'] / total_entries if total_entries > 0 else 0

    # Durchschnittlichen Score und Affiliate Links pro Position berechnen
    avg_pos_scores = {pos: (data['score_sum'] / data['count']) for pos, data in overall_results['pos_scores'].items()}
    avg_pos_affiliate_links = {pos: (data['affiliate_links_sum'] / data['count']) for pos, data in overall_results['pos_affiliate_links'].items()}
    ...
    # Position mit den absolut meisten Affiliate Links finden
    max_pos_total_affiliate_links = max(overall_results['pos_affiliate_links'], key=lambda pos: overall_results['pos_affiliate_links'][pos]['affiliate_links_sum'], default=None)
    max_total_affiliate_links = overall_results['pos_affiliate_links'].get(max_pos_total_affiliate_links, {}).get('affiliate_links_sum', 0)

    # Schreibe die Gesamtergebnisse ohne Suchmaschinen-Unterscheidung
    file.write(f"Gesamtauswertung (ohne Suchmaschinen-Unterscheidung):\n")
    file.write(f"Durchschnittlicher Score: {average_score:.2f}\n")
    file.write(f"Durchschnittliche Affiliate Links: {average_affiliate_links:.2f}\n")
    file.write(f"Durchschnittliche Hervorhebungsstärke: {average_highlight_strength:.2f}\n")
    file.write(f"Durchschnittliche Position: {average_position:.2f}\n")
    ...
    # Schreibe die avg_pos_scores und avg_pos_affiliate_links detailliert in die Datei
    file.write(f"\nDurchschnittliche Scores pro Position (gesamt):\n")
    for pos, avg_score in avg_pos_scores.items():
        file.write(f"Position {pos}: {avg_score:.2f}\n")
    
    file.write(f"\nDurchschnittliche Affiliate Links pro Position (gesamt):\n")
    for pos, avg_aff_links in avg_pos_affiliate_links.items():
        file.write(f"Position {pos}: {avg_aff_links:.2f}\n")
    
    file.write(f"\nAffiliate Links pro Position (gesamt):\n")
    for pos, data in overall_results['pos_affiliate_links'].items():
        file.write(f"Position {pos}: {data['affiliate_links_sum']} Affiliate Links (in {data['count']} Einträgen)\n")
    
    file.write(f"\nTotal entries (gesamt): {total_entries}\n")
